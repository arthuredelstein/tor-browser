...
// to be used with SpawnTask.js. Bug 1193593.
let testMediaQueriesInPictureElements = function* (resisting) {
  ...
  for (let [key, offVal, onVal] of expected_values) {
    ...
    if (expected) {
      ...
      lines += " <img id ='" + key + ":" + expected + "' class='testImage' src='/tests/layout/style/test/chrome/mismatch.png' alt='" + key + "' />\n";
      lines += "</picture><br/>\n";
    }
  }
  ...
  for (let testImage of testImages) {
    ok(testImage.currentSrc.endsWith("/match.png"), "Media query '" + testImage.id + "' in picture should match.");
  }
};
