...
// ("privacy.firstparty.isolate" pref is set to 2) then when a loaded file is cached,
...
// __Constants__.
let privacyPref = "privacy.firstparty.isolate",
...
// one by one.
add_task(function* () {
  // Test the pref "privacy.firstparty.isolate" both enabled and disabled
  for (let isolate of [true, false]) {
    ...
    yield SpecialPowers.pushPrefEnv({"set": [[privacyPref, isolate]]});
    // Open test tabs
    let tabs = [];
