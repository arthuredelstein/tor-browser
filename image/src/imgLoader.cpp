...
{
  if (RemoveMatchingUrlsFromCache(ImageURL(uri)))
    return NS_OK;
...
NS_IMETHODIMP imgLoader::FindEntryProperties(nsIURI *uri, nsIProperties **_retval)
{
  ...
  *_retval = nullptr;
  ...
  for (uint32_t i = 0; i < entries.Length(); ++i) {
    ...
    if (request) {
      request->mURI->ToIURI()->Equals(uri, &isEqual);
      if (isEqual) {
...
bool imgLoader::SetHasProxies(nsIURI *firstPartyURI, ImageURL *imgURI)
{
  VerifyCacheSizes();
  ...
  nsAutoCString key = GetCacheKey(firstPartyURI, imgURI->ToIURI(), nullptr);
  nsRefPtr<imgCacheEntry> entry;
...
bool imgLoader::RemoveMatchingUrlsFromCache(nsIURI *aImgURI)
{
  ...
  for (uint32_t i = 0; i < entries.Length(); ++i) {
    ...
    entries[i]->mRequest->mURI->ToIURI()->Equals(aImgURI, &isEqual);
...
bool imgLoader::RemoveMatchingUrlsFromCache(ImageURL *aImgURI)
{
  if (!aImgURI) return false;
  return RemoveMatchingUrlsFromCache(aImgURI->ToIURI());
}

bool imgLoader::RemoveFromCache(nsCString& key,
                                imgCacheTable &cache,
                                imgCacheQueue &queue)
{
			      ...
			      imgRequestProxy **_retval)
{
  ...
  *_retval = nullptr;
  ...
  if (!request) {
    ...
    request->Init(aURI, aURI, aFirstPartyURI, channelLoadGroup, newChannel, entry, aCX,
