...
function printAttrs(name, attrs) {
  do_print(name + " {\n" +
           ...
           "\tfirstPartyUri: '" + attrs.firstPartyUri + "'\n}");
}
...
function checkValues(attrs, values) {
  ...
  do_check_eq(attrs.firstPartyUri, values.firstPartyUri || '');
}
...
function run_test() {
  ...
  // First party Uri
  var exampleOrg_firstPartyUri = ssm.createCodebasePrincipal(makeURI('http://example.org'), {firstPartyUri: 'example.org'});
  checkOriginAttributes(exampleOrg_firstPartyUri, { firstPartyUri: "example.org" }, '^firstPartyUri=example.org');
  do_check_eq(exampleOrg_firstPartyUri.origin, 'http://example.org^firstPartyUri=example.org');
  ...
  checkCrossOrigin(exampleOrg_firstPartyUri, exampleOrg);
  ...
  var tests = [
    ...
    [ "^firstPartyUri=example.org", {firstPartyUri: "example.org"} ],
  ...
  var dflt2_tests = [
    [ "", {}, {}, "" ],
    [ "^firstPartyUri=foo.com", {firstPartyUri: "foo.com"}, {}, "" ],
    [ "^appId=5", {appId: 5}, {appId: 5}, "^appId=5" ],
    [ "^appId=5&firstPartyUri=foo.com", {appId: 5, firstPartyUri: "foo.com"}, {appId: 5}, "^appId=5" ] ];
