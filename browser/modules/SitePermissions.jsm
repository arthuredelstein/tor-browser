...
/**
 * A helper function for keying temporary permissions and global permissions.
 *
 */
const _keyForPrincipal = function(principal) {
  let originNoSuffix = principal.originNoSuffix;
  // Defensively make sure we own the attrs instance.
  let attrs = ChromeUtils.createOriginAttributesFromOrigin(principal.originSuffix);
  // We don't isolate by container, but we do isolate by first party
  // and private browsing.
  // (attrs.firstPartyDomain is empty unless
  // privacy.firstparty.isolate is enabled.)
  attrs.userContextId = 0;
  let suffix = ChromeUtils.originAttributesToSuffix(attrs);
  return originNoSuffix + suffix;
};
