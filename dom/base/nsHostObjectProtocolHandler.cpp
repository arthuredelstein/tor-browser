                                          ...
                                          nsACString& aUri)
{
  ...
  info->mFirstPartyHost = isolationKey;
                                             ...
                                             const nsACString& isolationKey)
{
  if (gDataTable) {
    ...
    if (info->mFirstPartyHost == isolationKey) {
      nsCString uriIgnoringRef;
      int32_t hashPos = aUri.FindChar('#');
      if (hashPos < 0) {
        uriIgnoringRef = aUri;
...
static nsISupports*
GetDataObject(nsIURI* aURI, const nsACString& isolationKey)
{
  nsCString spec;
  aURI->GetSpec(spec);
  ...
  return (info && info->mFirstPartyHost == isolationKey)
                                         ...
                                         nsIChannel** result)
{
  *result = nullptr;
  ...
  if (aLoadInfo) {
    ...
    firstPartyHost = ThirdPartyUtil::GetFirstPartyHost(loadingDocument);
  }
