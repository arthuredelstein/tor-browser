...
bool IsImageExtractionAllowed(nsIDocument *aDocument, JSContext *aCx)
{
    ...
    if (isThirdParty) {
        ...
        nsContentUtils::LogMessageToConsole("Blocked third party %s from extract canvas data.", docURISpec.get());
        if (isScriptKnown) {
            nsContentUtils::LogMessageToConsole("Canvas image extraction attempt by script %s:%u.",
    ...
    if (permission == nsIPermissionManager::ALLOW_ACTION) {
        ...
        nsContentUtils::LogMessageToConsole("Denied %s permission to extracting canvas data.", firstPartySpec.get());
        if (isScriptKnown) {
            nsContentUtils::LogMessageToConsole("Canvas image extraction attempt by script %s:%u.",
                                                scriptFile.get(), scriptLine);
        }
